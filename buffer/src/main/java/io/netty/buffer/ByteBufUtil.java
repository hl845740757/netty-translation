...
/**
 * 与处理{@link ByteBuf}相关的工具方法的集合。
 * 例如：生成16进制的转储文件(堆栈信息)，和交换整数的字节序。
 * <p></p>
...
public final class ByteBufUtil {
    ...
    /**
     * 用于确定{@link ByteBuf#ensureWritable(int, boolean)}的返回值是否意味着这里有充足的空间并且一个写操作将会成功。
     * <p></p>
     ...
     * will succeed.
     *         当{@code ensureWritableResult}意味着有充足的空间并且写操作将成功时返回true。
     */
    ...
    /**
     * 计算指定ByteBuf的hashcode，该方法在实现一个新的buffer类型时很有用。
     *
    ...
    /**
     *
     * @param haystack 这个参数名很有意思啊，大海捞针...
     *
    ...
    /**
     * 当且仅当指定的两个buffer在指定区间段和另一方相同时返回true。
     * 一个更加简洁的表示方式就是：
     * <p>
    ...
    /**
     * 当且仅当两个给定的buffer都等于另一方时返回true，就像{@link ByteBuf#equals(Object)}描述的那样.
     * <p></p>
     *
     * Returns {@code true} if and only if the two specified buffers are
